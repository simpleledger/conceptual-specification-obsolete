...
This specification defines a protocol for using a single Bitcon Cash address as a simple ledger to keep track of resources and data.  The protocol can be used in a number of modes.  In one mode, the allocation of multiple resources can be tracked between a number of entities. In other modes, monikers or label data can be associated with bitcoin cash addresses. The protocol is designed to be a human-readable payload when decoded from bytes to ASCII to improve overall ledger transparency and auditability.  Some encryption options will be provided for user privacy protection.

Here is a brief description of each of the SLP modes:

* **Numerical ledger mode:**  In the `numerical` SLP mode quantifiable resources can be created and transfered between entities using a series of OP_RETURN transactions outputs from the same Bitcoin Cash address.  That series of transactions can be combined to arrive at the ledger's current numerical state.  The SLP ledger can be shared with others for their inspection by simply sharing the ledger's bitcoin address and using a web based SLP implementation.

* **Moniker and Label ledger mode (e.g., native wallet labeling system):** In the `moniker` SLP mode a unique name can be associated with a particular bitcoin cash address.  This moniker mode could be used to create your own moniker/address datastore, where the datastore is identified by the SLP ledger's address.  Any protocol or software could use SLP for associating unique monikers with bitcoin addresses.  This could provide a DNS-like system for bitcoin addresses that a wallet software could adopt making it easy to memorize addresses.

* In the `label` SLP mode a there is no uniqueness requirement in the name field.  For example, you could have a bitcoin wallet that dedicates one of its addresses as an SLP ledger that will be used to creating names and labels for all of the other addresses in the wallet.  This same dedicated SLP ledger within the wallet could also be used for creating unique transaction labels.  For some scenarios, like in a wallet, the names/labels fields could be encrypted using the SLP ledger address's public key in order to protect privacy.  This would elliminate the need to use cloud file based-storage for labeling like Trezor relies upon.

* **Lookup ledger mode**: In the `lookup` SLP mode the SLP ledger address would be used to query for OP_RETURN SLP data that has been  stored by each of the addresses listed within the SLP ledger.  For example, if you were running a centrally controlled voting center you could could register your voters' addresses within your SLP address, and then the voters could post their votes from their own address using an OP_RETURN output like: `SLP for=<target-slp-ledger-address> vote=<their voting data here>`.  This voting system using SLP would allow a central authority to verify voter identity and register all voters at the SLP address, but the each voter would vote using their own bitcoin address.

* Another `lookup` mode example would be to use an SLP ledger as a DNS datastore for a particular TLD, where the SLP ledger would hold a list of bitcoin addresses and respective domain names, but the IP address for the server would be published by the owners bitcoin address using something like: `SLP for=<target-slp-ledger-address> ip4=<ip4 addr> ip6=<ip6 addr>`.

* **As a ledger for creating "tokens":** In this SLP mode the ledger can be used as a method to issue and transfer tokens, create and maintain the state of a virtual blockchain that contains one or more digital assets.  These goals can be accomplished by using an internal SLP ledger to track creation, destruction, and transfer of digital assets between the public keys or addresses holding an asset.  In a strict digital asset mode, signatures can be required during asset transfers initiated by the holder.

As part of the SLP project a public registry will be created to where SLP ledgers can publish their address if desired and create a unique moniker for the SLP ledger.  The public registry should simplify usage of SLP interoperability between assets utilizing SLP and any wallet or web service following SLP can check current state for any SLP asset.  The registry itself will be stored within a SLP ledger. During the registration process each SLP ledger's address will be registered and a unique moniker may be provided.  This public registry will be relied upon by virtual blockchains for promoting interoperability between virtual chains. SLP can work without the registry, but the registry will provide a unique moniker, analogous to a DNS, to protect the holder in case the SLP ledger address is changed.
